...
def get_row_stitch_sequences(connectivity, creases) -> list[list[str]]:
    ...
    """
    return [dtw_to_stitches(dtw, bflo)[::(-1) ** i] for i, (dtw, bflo) in enumerate(zip(connectivity, creases))]


def get_folded_rows(row_stitches, connectivity) -> list[str]:
